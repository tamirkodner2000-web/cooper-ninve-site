...
function agentsSections() {
  const agentBenefits = [
    ["הסוכן נשאר מול הלקוח", "קופר נינוה פועלת כגב חיתומי מקצועי, מבלי להחליף את מערכת היחסים שבין הסוכן למבוטח."],
    ["פתרון לסיכונים לא סטנדרטיים", "בחינת סיכונים מורכבים או לא שגרתיים מול שווקים וחתמים רלוונטיים."],
    ["גישה לשווקים בינלאומיים", "חיבור לשווקי ביטוח מעבר לים, בהתאם לתחום הסיכון, תיאבון החיתום והסמכויות הרלוונטיות."],
    ["תהליך חיתום מסודר", "ריכוז שאלונים, מסמכים ונתונים חיתומיים בצורה שמקדמת בחינה מקצועית של הסיכון."],
    ["הפקה וליווי בעברית", "פוליסות, מסמכים, שירות וליווי מקצועי בעברית לאורך חיי הפוליסה."],
  ];
  return `
    ${agentJourneySection()}
    <section class="section"><div class="container split-band"><div><h2>כשתיק ביטוח לא נכנס למסלול הרגיל — קופר נינוה נכנסת לתמונה</h2><p>קופר נינוה עובדת עם סוכני ביטוח כשותף חיתומי מאחורי הקלעים — מסייעת בבחינת סיכונים מורכבים, התאמתם לשווקים רלוונטיים, הפקת פוליסות בעברית וליווי מקצועי לאורך חיי הפוליסה, תוך שמירה על מקומו המרכזי של הסוכן מול הלקוח.</p></div><ul class="feature-list">${agentBenefits.map(([title, text]) => `<li><strong>${title}</strong><p>${text}</p></li>`).join("")}</ul></div></section>
